...
document.querySelector('#submit').addEventListener('click', () => {
    ...
    todo_done_el.addEventListener('click', () => {
        // Mark input as done (style with CSS)
        todo_input_el.classList.add('done');

        // Remove current action icons
        todo_actions_el.innerHTML = '';

        // Create new delete icon
        const done_delete_el = document.createElement('i');
        done_delete_el.classList.add('fa-solid', 'fa-trash');

        // Append only the delete icon
        todo_actions_el.appendChild(done_delete_el);

        // Delete functionality for this new delete icon
        done_delete_el.addEventListener('click', () => {
            todo_el.classList.add('slide-out');
